...
void outputsibl(pnode current, int modifymove)
{
	if(p1==NULL) return;
	cout<<"MOVE: ("<<p1->m[0]<<","<<p1->m[1]<<") Utility="<<p1->u<<endl;
	if(modifymove==p1->u) {nextmove[0]=p1->m[0];nextmove[1]=p1->m[1];}
	outputsibl(p1->nsibl,modifymove);
}
void outputtree(pnode current, int modifymove,int minmax)
{
	...
	p1=current->fchild;
	if(p1==NULL) return;
	if(minmax) cout<<"MAX["<<role<<"]  Branch of node [movement: ("<<'X'+'O'-role<<")("<<current->m[0]<<","<<current->m[1]<<")]";
	else cout<<"MIN["<<'X'+'O'-role<<"]  Branch of node [movement: ("<<role<<")("<<current->m[0]<<","<<current->m[1]<<")]";
	outputsibl(p1,(modifymove)?current->u:-999);
...
int main(int argc, char *argv[])
{
	...
	if(ifp=fopen(argv[1])==NULL){cout<<"Can't open the file"<<endl;exit(1);} //open file
	
	fscanf(ifp,"SIZE: %d",&n);
	fscanf(ifp,"PLAYER: %c",&role);
	if(role!='X' && role!='O') {cout<<"File Format Error"<<endl;exit(1);}
	for(i=0;i<n;i++) for (j=0;j<n;j++)
	{
	...
	cout<<endl;
	cout<<"Computer Choose To Move: ("<<nextmove[0]<<","<<nextmove[1]<<")"<<endl;
	...
	cout<<endl;
	cout<<"Now, it's your turn, input two integers to move"<<endl<<"example: [1 1] to put your next piece into the left top box"<<endl;
	cin>>i>>j;
	while((i<=0 || i>n) || (j<=0 || j>n) || board.p[i][j]!='-') {cout<<"Illegal move, try again."<<endl;cin>>i>>j;}
	board.p[i][j]='O'+'X'-role;
